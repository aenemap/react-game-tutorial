...
const verifyPlayer = (token, cb) => {
  ...
  console.log('verifyPlayer => uncheckedToken => ', uncheckedToken)
  const kid = uncheckedToken.header.kid
  console.log('verifyPlayer => kid => ', kid)

  client.getSigningKey(kid, (err, key) => {
    console.log('getSigningKey => key => ', key)
    console.log('getSigningKey => err => ', err)
    const signingKey = key !== undefined ? key.publicKey : key.rsaPublicKey
